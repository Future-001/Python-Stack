...
内置函数： int 取整
        ...
        zip :  拉链法：*****
         ...
         min()   max()   :  返回最小值或者最大值   ******************
                        ...
                        如果可以加入 key : 那么将可迭代对象的元素按照顺序传输 key 对应的 函数中
                                    获取到返回值后，在进行比较
                        way 1:
                                l1 = [1,2,99,33,-4,-8,-1]
                                for i in l1:
        ...
        filter :  过滤 筛选， 类似于列表推导式的筛选模式。
                    返回一个迭代器：  filter(lambda x:x>3,l1)  返回的 迭代器中的结果是满足条件的 x
                                具体看一下下面的例子代码理解一下
                    列表推导式 返回的是一个 列表。

        map :  列表推导式的 循环模式  返回的是一个迭代器。
                    map(lambda i:i**2,range(1,6)  返回的迭代器中的结果 保存的是返回值
...
# 筛选器，筛选器，筛选的是满足结果的 i ，所以最后得到的就是 i  所以最后输出 1 2 3
