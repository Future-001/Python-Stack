...
形参角度：
        ...
        仅限关键字参数：
                    只能接受关键字参数， 放在 *args 和 **kwargs中间，一定只能是 关键字参数。一定要给他赋值，不然会报错。

        形参的最终顺序：
                    * args 的位置，一定放在 位置参数后面  但是一定也要注意，默认参数的覆盖问题。 默认参数一定要放在最后。
...
名称空间角度：
                    在python解释器开始执行之后, 就会在内存中开辟一个空间, 每当遇到一个 变量 的时候,
                     就把变量名和值之间的关系记录下来, 但是当遇到 函数定义 的时候, 解释器只是把 函数名 读入内存,(函数名与内存地址的对应关系）
...
# def order(a,b,sex='m',*args,age,**kwargs):  因为如果只用关键字参数，当你使用位置参数，没有写出关键字，那么会被覆盖了，，，，尽量放在 *args后面
...
# 会导致sex得到多个参数，也会报错。
# order(1.txt,2,4,5,6,sex='F',name='sga',agsw='sdgo',age=18,c=99)       这样也会出错，因为 sex得到了多个参数
