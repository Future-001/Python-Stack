...
member_information_path = os.path.abspath('member_information\博客园信息表.txt')
...
def register():
    if status == False:
        ...
        while True:
            ...
            pwd = input('请输入密码:')
            f1 = open(member_information_path, mode='rb')
            f1.seek(0)
            if usr.isalnum():
                while True:
                    try:
                ...
                if Exist_status == False:
                    if 6<=len(pwd)<=14:
                        f = open(member_information_path,mode='ab')
                        f.seek(0,2)
                        md5 = hashlib.md5(usr.encode('utf-8'))
                        md5.update(pwd.encode('utf-8'))
                        pickle.dump(usr + '|' + str(md5.digest())+'\n', f)
                        f.close()
                        usr_file_path = os.path.join(usr_file_path,f'{usr}')
                        os.mkdir(usr_file_path)
                        return
                    else:
...
def login():
    ...
    if status==False:
        print('欢迎来到登录页面')
        with open(member_information_path,mode='rb') as f :
            count = 3
            while True:
                if count>0:
                    f.seek(0)
...
def article():     # 取巧了，用户名直接变成了全局的。。。。
    usr_file_path = os.path.dirname(__file__) + r'\usr_file'  # 为什么无法获取到全局的地址。。。。不是很清楚
    if status:
...
请输入你所要进行的操作；""")
            if choice.isdecimal():
                ...
                if choice == 1:
                    ...
                    article_path =os.path.join(usr_file_path,usr,'article')
                    ...
                    import_path = os.path.join(usr_file_path,usr,'article')
...
def comments():
    if status:
        ...
        usr_list = os.listdir(usr_file_path)
        ...
        while 1:
            ...
            if usr_num.isdecimal():
                ...
                if usr_num in range(len(usr_list)) :
                    usr_article = os.listdir(os.path.join(usr_file_path,usr_list[usr_num],'article'))
                    print('文章内容如下： ')
                    ...
                    while 1:
                        ...
                        if choice_article.isdecimal():
                            ...
                            if choice_article in range(len(usr_article)):
                                article_path = os.path.join(usr_file_path,usr_list[usr_num],'article',usr_article[choice_article])
