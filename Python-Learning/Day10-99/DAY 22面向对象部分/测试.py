...
class Weapon:
    ...
    def 普攻(self,person2):
        person2.hp = person2.hp - self.owner.ad
        print('%s赤手空拳打了%s %s滴血，%s还剩%s血。' % (
            self.owner.name, person2.name, self.owner.ad,
            person2.name, person2.hp))
    def pan(self,person2):
        self.pan_ad=20
        person2.hp = person2.hp - self.pan_ad
        print('%s使用%s打了%s %s滴血，%s还剩%s血。'%(
              self.owner.name,'平底锅',person2.name,self.pan_ad,
              person2.name,person2.hp))
    def ax(self ,person2):
        self.ax_ad=50
        person2.hp = person2.hp - self.pan_ad
        print('%s使用%s砍伤了%s %s滴血，%s还剩%s血。'%(
              self.owner.name, '斧头', person2.name, self.ax_ad,
              person2.name, person2.hp))
    def rod(self,person2):
        self.rod_ad=65
        person2.hp = person2.hp - self.pan_ad
        print('%s使用%s打了%s %s滴血，%s还剩%s血。'
                %(self.owner.name, '双截棍', person2.name, self.rod_ad,
                person2.name, person2.hp))
...
class Car():
    ...
    def bike(self):
        self.bike_speed=60
        print('%s骑着%s开着%s迈的车行驶在赛道上。' % (self.owner.name, '自行车', self.bike_speed))

    def yamh(self):
        self.yamh_speed=80
        print('%s骑着%s开着%s迈的车行驶在赛道上。'% (self.owner.name, 'yamh', self.yamh_speed))
    def beemar(self):
        self.beemar_speed=120
        print('%s骑着%s开着%s迈的车行驶在赛道上。' %(self.owner.name, '宝马', self.beemar_speed))
