...
编码的进阶：
        ASCII gbk unicode uft-8 big5.......二进制与文字的对应关系。   最核心的 Unicode   与所有的编码方式都有一个对应关系。
        所有的编码方式（除去Unicode）不能互相识别。
        在内存中，所有的数据都是Unicode编码的（int bool str list dict tuple set)，除去 bytes 型。
                   但是 bytes  只能和  str 进行转换。。。。。。，别的Unicode的数据类型必须要先转换成str 才能转化为bytes类型。
...
文件操作初识：
        1.txt.护士空姐少妇的联系方式.txt
        利用 python 代码写一个很 low 的软件取操作一个文件。
            文件路径： path
            打开方式：mode :   读，写，追加，读写，写读......
            编码方式：encoding   utf-8 , gbk, gb2312
        f1 = open(path,encoding='utf-8',mode='r)     #  encoding这里表示文件打开的编码方式  一定要与原来文件中内容的 编码方式一致。
        f1.read()    f1.close()

        open 内置函数，open底层调用的是操作系统的接口，
        f1 变量，f1,fh,file_handler,f_h  文件句柄  对文件进行的任何操作都得通过文件句柄
        encoding : 可以不写，默认的编码本： 操作系统的默认编码。   encoding 的对用编码方式要与需要打开文件中内容的编码方式一致才能打开。
                windows: gbk
                linux: utf-8
                mac: utf-8
        f1.close()  关闭文件句柄

    打开文件错误的常见几种方式： ‘D:\Code’  ‘D:\\Code’ 再加上一个 \  因为原来的字符可能被识别为转义字符去了，所以再加上一个 \解决
...
常见的文件操作：
        读：
            ...
            for  遍历其中的每一行元素
                    #  有了 readlines()为什么还需要 for 呢？  因为文件句柄相当于一个迭代器，当读取到这一行后，这一行就消失，
        ...
        写：
            ...
            .wirte()
              ...
              # 本来在文件中进行编码，是以字符串方式 Unicode，存储时利用bytes类型，涉及到一次编码的转换问题，但是open 这个函数已经帮你弄好了
