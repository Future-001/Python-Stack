...
print()
"""
面试题；      同一代码块使用一个缓存机制，不同代码块之下利用的是不同的缓存机制 （小数据池），注意小数据池的范围以及缓存机制的优点。
...
不同代码快下的缓存机制：  （小数据池）
    python自动将-5~256的整数进行了缓存，当你将这些整数赋值给变量时，并不会重新创建对象，而是使用已经创建好的缓存对象（注意好整数的范围）
    python将一定规则的字符串留在 字符串驻留池 中，创建一份，当你将这些字符串赋值给变量时，并不会重新创建对象，而是使用在字符串驻留池中创建好的字符串
    ...
    # 字符串:
    #     1.txt.字符串在做乘法的时候总长度不能超过20,进行驻留   # 记住
...
# 首先，对比两个字典，数值是一样的，但是，由于字典是可变的，不满足 同一代码块下的缓存机制（ int bool 几乎全部字符串） 。所以，首先二者的地址一定不同，
...
# s={[1.txt,2,3],{'name':'kevin'}}    unhashable type: 'list'
# print(s)
"""
...
print('============ .pop() 随即删除，因为集合无序，所以不能按照索引 ============')
print(s1.pop())
print(s1)
print()
...
l5[-1].append(90)
print(l4,l5)   # [1.txt, 2, 3, [33, 44, 90], 666] [1.txt, 2, 3, [33, 44, 90]]
l4[3].remove(33)
print(l4,l5)
print()
