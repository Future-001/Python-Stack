                    ...
                    形参：
                        ...
                        默认值参数：
                                ...
                                用于形参处，设定好默认参数
                                           ...
                                           可以去看看 open 的，里面mode 默认读，你可以对其进行覆盖，但是原函数是不变的。
                                设置的意义：经常使用的才设置。
                                小心一点： 在使用默认参数时，注意一下 万一位置不对应，没有关键字 有的地方被覆盖了，导致后面彼得参数没传进去就报错

三元运算符
 a = b if 1.txt>2 else c
...
ret = momo(145,170,sex='女',age='23',skill='great')    # 实参    按照位置传入参数，从左至右，一一对应  有了等号，就是关键字传入参数
